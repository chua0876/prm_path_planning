...
def PRM_planning(x_start, y_start, x_goal, y_goal, env, pt_size):
  ...
  return roadmap, sample_x ,sample_y, px ,py, obstacle_kdtree
...
class Node: # Node class for Dijkstra's Algorithm
...
map,x,y,px,py,obstacle_kdtree = PRM_planning(x_start, y_start, x_goal, y_goal, env, pt_size)
#plt_roadmap(map,x,y)
pl.scatter(x,y)
print("Number of points: " + str(len(px)))
print(py)
pl.plot(px,py, "-g", linewidth = 3.0)
pl.pause(10)
...
"""
def post_path(px, py, pt_size, obstacle_kdtree):
    for rep in range(50):
        # To avoid deleting the start and goal points
        # px_middle = px[1:-2]
        x_random = random.sample(px, 2)  # Pick two random points along the path
        x0_i = px.index(x_random[0])
        x1_i = px.index(x_random[1])
        y_random = [py[x0_i], py[x1_i]]
        if not collision(x_random[0],y_random[0],x_random[1],y_random[1],pt_size,obstacle_kdtree):  # Check if collision-free
            if x1_i > x0_i:
                t = 1
                while (x0_i + t) < x1_i:
                  if px[x0_i + t] != px[-1]:
            ...
            continue
    new_px, new_py = px, py
    return new_px, new_py # Return new path

##### Calling Post-processing for Original Path #####
new_x, new_y = post_path(px, py, pt_size, obstacle_kdtree)
print("Number of points after shortcutting: " + str(len(new_x)))
print(new_y)
pl.plot(new_x, new_y, "-b", linewidth = 3.0)
pl.pause(1000)